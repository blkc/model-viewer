...
func _ready():
    ...
    var current_tip_local_transform := Model.get_bone_pose_transform(armature, tip_index)
    var current_magnet_local_transform := Model.get_bone_pose_transform(armature, magnet_index)
    ...
    new_tip.transform = current_tip_local_transform

    new_magnet = Node3D.new()
    add_child(new_magnet)
    new_magnet.name = name + "_controller_magnet_target"
    new_magnet.transform = current_magnet_local_transform
    ...
    self.target = new_tip.transform
    self.magnet = new_magnet.position
...
func _on_skeleton_updated():
    if ik_group.is_tip_idle:
        var current_bone_tip_transform = Model.get_bone_pose_transform(armature, ik_group.tip_index)
        new_tip.transform = current_bone_tip_transform
    else:
        self.target = new_tip.transform

    if ik_group.is_magnet_idle:
        var current_bone_magnet_transform = Model.get_bone_pose_transform(armature, ik_group.magnet_index)
        new_magnet.transform = current_bone_magnet_transform
    else:
        self.magnet = new_magnet.position
